...
log_root = '/home/s4686009/remotedata/dSprites/logs'
...
shutil.copytree('.', os.path.join(log_path, 'src_dsprite') )

# shutil.copytree('.', os.path.join(log_path, 'src_dSprites') )

dsets = {"train": ImageList(open(source_path1).readlines()+open(source_path2).readlines(), transform=data_transforms["train"]), #ImageList(open(source_path2).readlines(), transform=data_transforms["train"],
...
def train_erm(optimizer,iter_train,iter_val):
    ...
    for iter_num in range(1, num_iter + 1):
        ...
        labels4 = labels_train[:, 3]
        labels1 = labels1.unsqueeze(1)
        labels3 = labels3.unsqueeze(1)
        labels4 = labels4.unsqueeze(1)

        labels_train = torch.cat((labels1, labels3, labels4), dim=1)
...
def train_mixup(optimizer,iter_train,iter_val):
    ...
    for iter_num in range(1, num_iter + 1):
        ...
        labels4_ = labels_train2[:, 3]
        labels1_ = labels1_.unsqueeze(1)
        labels3_ = labels3_.unsqueeze(1)
        labels4_ = labels4_.unsqueeze(1)
        labels_train = torch.cat((labels1, labels3, labels4), dim=1)
        labels_train = labels_train.float()
        labels_train_ = torch.cat((labels1_, labels3_, labels4_), dim=1)
        labels_train_ = labels_train_.float()
        ...
        total_loss = regression_loss + args.svd_weight*loss_svd+args.std_weight*loss_std
