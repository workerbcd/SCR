...
class ImageList(object):
    ...
    def __init__(self, image_list, labels=None, transform=None, target_transform=None,
        ...
        self.loader = loader
        self.root = '/home/s4686009/remotedata/MPI3D'
    ...
    def get_sample(self, prev_idx, UseKDE=False, y1=None,bandwidth=0.5):
        d_idx = list(range(len(self.imgs)))

        if UseKDE:
            target_list = self.imgs[:][1]

            kde_patch = KernelDensity(kernel='gaussian', bandwidth=bandwidth).fit(
                [[y1.cpu()]])  # should be 2D
            domain_each_rate = np.exp(kde_patch.score_samples(target_list))
