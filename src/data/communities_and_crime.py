...
def get_CommunitiesAndCrime_data_packet(args,path = './data/CommunitiesAndCrime/'):
    ...
    # assay
        ...
        y_domain = [i for i, sub in enumerate(assay2idx_list) for _ in sub]
    ...
    if args.is_ood == True:
        y_domain = y_domain[:, np.newaxis]
        data_packet['domain_y_train'] = y_domain[train_idx]
        data_packet['domain_y_valid'] = y_domain[valid_idx]
        data_packet['domain_y_test'] = y_domain[test_idx]
